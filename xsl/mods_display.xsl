...
<!--			</oai_dc:dc> -->
	...
	<!-- <dc:title> -->
		<div class="metadataRow metaSetCore"><div class="metadataLabel">
			<xsl:choose>
	...
	<xsl:template match="mods:classification">
		<div class="metadataRow metaSetAccess"><div>
...
<!-- 	<xsl:template match="mods:subject[mods:topic | mods:name | mods:occupation | mods:geographic | mods:hierarchicalGeographic | mods:cartographics | mods:temporal] "> -->
	  ...
	  <xsl:for-each select="mods:hierarchicalGeographic">
	  	<div class="metadataRow metaSetContent"><div>
	  ...
	  <xsl:if test="normalize-space(mods:cartographics)">
	    <xsl:for-each select="mods:cartographics/*">
	    	<div class="metadataRow metaSetContent"><div><xsl:value-of select="$subjectCartographic"/></div><div>
		  <xsl:value-of select="."/>
	      </div></div>
	    </xsl:for-each>
	  </xsl:if>
	  <xsl:if test="normalize-space(mods:temporal)">
	    <xsl:if test="mods:temporal">
	    	<div class="metadataRow metaSetContent"><div><xsl:value-of select="$subjectTemporal"/></div><div>
		  <xsl:for-each select="mods:temporal">
		    <xsl:value-of select="."/>
		    <xsl:if test="position()!=last()">-</xsl:if>
		  </xsl:for-each>
		...
		<xsl:if test="*[1][local-name()='topic'] and *[local-name()!='topic']">
			<div class="metadataRow metaSetContent"><div><xsl:value-of select="$subjectLocalname"/></div><div>
	...
	<xsl:template match="mods:tableOfContents">
	  ...
	  <xsl:if test="normalize-space(.)">
	  	<div class="metadataRow metaSetContent"><div><xsl:value-of select="toc"/></div><div>
			<xsl:value-of select="."/>
		  </div></div>
	  </xsl:if>
	</xsl:template>
...
<xsl:template match="mods:originInfo">
	...
	<xsl:if test="mods:dateCreated">
		<div class="metadataRow metaSetCore">
			<div class="metadataLabel"><xsl:value-of select="$dateCreated"/></div>
			<div><xsl:value-of select="mods:dateCreated"/>
	...
	<xsl:if test="mods:dateIssued">
		<div class="metadataRow metaSetCore">
			<div class="metadataLabel"><xsl:text>Date Published</xsl:text></div>
			<div><xsl:value-of select="mods:dateIssued"/>
...
-->
	...
	<xsl:template match="mods:typeOfResource">
		<div class="metadataRow metaSetCarrier">
			<div class="metadataLabel"><xsl:value-of select="$typeOfResource"/></div>
...
-->
	...
	<xsl:template match="mods:mimeType">
		  <xsl:if test="normalize-space(.)">
		  	<div class="metadataRow metaSetCarrier">
		  		<div><xsl:value-of select="$mimeType"/></div>
		  		<div><xsl:value-of select="."/></div>
		  	</div>
		  </xsl:if>
	</xsl:template>
